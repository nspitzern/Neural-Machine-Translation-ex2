...
import config_part1
from utils import plot_graph, timeit


def predict(decoder, trg, enc_output, trg_idx2word, device=config_part1.DEVICE):
    # make predictions
    prediction = [config_part1.START_TOKEN]

    # do translation for each word
    dec_hidden = decoder.init_hidden(device)

    dec_input = trg[0]

    for i in range(config_part1.MAX_SENTENCE_LENGTH):
        dec_output, dec_hidden = decoder(dec_input, enc_output, dec_hidden)

        # get index of the next word
        dec_input = dec_output.argmax(axis=1)

        # convert to word
        prediction.append(trg_idx2word[dec_input.item()])

        # check if sentence reached the length limit
        if trg_idx2word[dec_input.item()] == config_part1.END_TOKEN:
            break
...
def evaluate(dev_data, encoder, decoder, criterion, trg_idx2word, dev_trg_text, device):
    encoder.eval()
    decoder.eval()
    ...
    with torch.no_grad():
        ...
        for src, trg in dev_data:
            ...
            enc_output = enc_output[-1].unsqueeze(1)

            # pass through the decoder
            dec_hidden = decoder.init_hidden(device)

            for i in range(target_length - 1):
                dec_output, dec_hidden = decoder(trg[i], enc_output, dec_hidden)

                sentence_loss += criterion(dec_output, trg[i + 1].unsqueeze(0))
...
@timeit
def train(train_data, dev_data, encoder, decoder, trg_idx2word, dev_trg_text, epochs, device):
    ...
    # init optimizers
    enc_opt = config_part1.OPTIMIZER(encoder.parameters(), lr=config_part1.LR)
    dec_opt = config_part1.OPTIMIZER(decoder.parameters(), lr=config_part1.LR)
    ...
    for epoch in range(1, epochs + 1):
        ...
        # evaluate model
        dev_loss, bleu_score = evaluate(dev_data, encoder, decoder, criterion, trg_idx2word, dev_trg_text, device)

        dev_losses.append(dev_loss)
        bleu_scores.append(bleu_score)

        print(f'[{epoch}/{config_part1.EPOCHS}] Training loss: {train_loss}, Validation loss: {dev_loss}, BLEU score: {bleu_score}')
        ...
        if bleu_score >= best_bleu:
            ...
            }, config_part1.MODEL_CHECKPOINT_PATH)
...
def main(args):
    ...
    train_src_lines = convert_rare_words(train_src_lines, min_occurrence_threshold=config_part1.MIN_OCCURRENCES_THRESHOLD)
    train_trg_lines = convert_rare_words(train_trg_lines, min_occurrence_threshold=config_part1.MIN_OCCURRENCES_THRESHOLD)
    ...
    dev_trg_lines = update_with_unknown(dev_trg_lines, trg_word2idx, unknown_word=config_part1.UNKNOWN_TOKEN)

    dev_trg_text = dev_trg_lines.copy()

    # create datasets
    train_dataset = SentencesDataSet(train_src_lines, train_trg_lines, src_word2idx, trg_word2idx, shuffle=True, device=device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--path', type=str, required=True, help='Path to train, test and dev files')
