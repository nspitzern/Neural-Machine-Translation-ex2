...
def predict(decoder, trg, enc_output, trg_idx2word, device=config_part2.DEVICE):
    # make predictions
    prediction = [config_part2.START_TOKEN]

    dec_input = trg[0]

    dec_hidden = decoder.init_hidden(device)
    # create initializing context
    dec_context = torch.zeros(1, 1, config_part2.HIDDEN_SIZE, device=device)

    for i in range(config_part2.MAX_SENTENCE_LENGTH):
        ...
        if trg_idx2word[dec_input.item()] == config_part2.END_TOKEN:
            break
...
def evaluate(dev_data, encoder, decoder, criterion, trg_idx2word, dev_trg_text, device=config_part2.DEVICE):
    encoder.eval()
    decoder.eval()
    ...
    with torch.no_grad():
        ...
        for src, trg in dev_data:
            ...
            for i in range(target_length - 1):
                dec_output, dec_hidden, dec_context, dec_attention = decoder(trg[i], enc_output, dec_hidden, dec_context)
...
def train(train_data, dev_data, encoder, decoder, trg_idx2word, dev_trg_text, epochs, device):
    ...
    # init criterion
    criterion = config_part2.CRITERION()

    # init optimizers
    enc_opt = config_part2.OPTIMIZER(encoder.parameters(), lr=config_part2.LR)
    dec_opt = config_part2.OPTIMIZER(decoder.parameters(), lr=config_part2.LR)
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            f'[{epoch}/{config_part2.EPOCHS}] Training loss: {train_loss}, Validation loss: {dev_loss}, BLEU score: {bleu_score}')
        ...
        if bleu_score >= best_bleu:
            ...
            }, config_part2.MODEL_CHECKPOINT_PATH)
...
def main(args):
    ...
    train_src_lines = convert_rare_words(train_src_lines,
                                         min_occurrence_threshold=config_part2.MIN_OCCURRENCES_THRESHOLD)
    ...
    dev_trg_lines = update_with_unknown(dev_trg_lines, trg_word2idx, unknown_word=config_part2.UNKNOWN_TOKEN)
    ...
    encoder = AttentionEncoder(input_size=len(src_word2idx),
                               embedding_size=config_part2.EMBEDDING_SIZE,
                               hidden_size=config_part2.HIDDEN_SIZE,
                               dropout=config_part2.DROP_RATE)
